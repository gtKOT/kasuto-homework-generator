...
import unittest
import HW_polyn3_2014_M1_summer as hw
...
class TestFracPolynI(unittest.TestCase):
    def test(self):
        self.assertEqual(
            '(x+2)+(x+3)',
            hw.frac_polynI('x', [0, 0, 0, 0, 0, 0], [1, 1, 2, 1, 1, 3], False)
        )
        self.assertEqual(
            '\myfrac{1}{2}(3y+4)+\myfrac{1}{5}(6y+7)',
            hw.frac_polynI('y', [0, 0, 0, 0, 0, 0], [2, 3, 4, 5, 6, 7], False)
        )
        self.assertEqual(
            '-(-z-2)-(-z-3)',
            hw.frac_polynI('z', [1, 1, 1, 1, 1, 1], [1, 1, 2, 1, 1, 3], False)
        )
        self.assertEqual(
            '-\myfrac{1}{2}(-3w-4)-\myfrac{1}{5}(-6w-7)',
            hw.frac_polynI('w', [1, 1, 1, 1, 1, 1], [2, 3, 4, 5, 6, 7], False)
        )
        self.assertEqual(
            '$-\myfrac{1}{2}(-3w-4)-\myfrac{1}{5}(-6w-7)$',
            hw.frac_polynI('w', [1, 1, 1, 1, 1, 1], [2, 3, 4, 5, 6, 7], True)
        )
...
# tenkaiAと同じ処理
class TestIntTenkaiA(unittest.TestCase):
    def test(self):
        self.assertEqual(
            'x+2+x+3',
            hw.int_tenkaiA('x', [0, 0, 0, 0, 0, 0], [1, 1, 2, 1, 1, 3], False)
        )
        self.assertEqual(
            '6y+8+30y+35',
            hw.int_tenkaiA('y', [0, 0, 0, 0, 0, 0], [2, 3, 4, 5, 6, 7], False)
        )
        self.assertEqual(
            '-z-2-z-3',
            hw.int_tenkaiA('z', [1, 0, 0, 1, 0, 0], [1, 1, 2, 1, 1, 3], False)
        )
        self.assertEqual(
            '-6w-8-30w-35',
            hw.int_tenkaiA('w', [1, 0, 0, 1, 0, 0], [2, 3, 4, 5, 6, 7], False)
        )
        self.assertEqual(
            '$-6w-8-30w-35$',
            hw.int_tenkaiA('w', [1, 0, 0, 1, 0, 0], [2, 3, 4, 5, 6, 7], True)
        )


# tenkaiと同じ処理
class TestIntTenkaiB(unittest.TestCase):
    def test(self):
        self.assertEqual(
            '2x+5',  # (x+2)+(x+3)
            hw.int_tenkaiB('x', [0, 0, 0, 0, 0, 0], [1, 1, 2, 1, 1, 3], False)
        )
        self.assertEqual(
            '36y+43',  # 2(3y+4)+5(6y+7)
            hw.int_tenkaiB('y', [0, 0, 0, 0, 0, 0], [2, 3, 4, 5, 6, 7], False)
        )
        self.assertEqual(
            '-2z+5',  # -(z-2)-(z-3)
            hw.int_tenkaiB('z', [1, 0, 1, 1, 0, 1], [1, 1, 2, 1, 1, 3], False)
        )
        self.assertEqual(
            '-36w+43',  # -2(3w-4)-5(6w-7)
            hw.int_tenkaiB('w', [1, 0, 1, 1, 0, 1], [2, 3, 4, 5, 6, 7], False)
        )
        self.assertEqual(
            '-2a',  # -(a+2)-(a-2)
            hw.int_tenkaiB('a', [1, 0, 0, 1, 0, 1], [1, 1, 2, 1, 1, 2], False)
        )
        self.assertEqual(
            '-4',  # -(b+2)-(-b+2)
            hw.int_tenkaiB('b', [1, 0, 0, 1, 1, 0], [1, 1, 2, 1, 1, 2], False)
        )
        self.assertEqual(
            '0',  # -(c+2)-(-c-2)
            hw.int_tenkaiB('c', [1, 0, 0, 1, 1, 1], [1, 1, 2, 1, 1, 2], False)
        )
        self.assertEqual(
            '$0$',
            hw.int_tenkaiB('c', [1, 0, 0, 1, 1, 1], [1, 1, 2, 1, 1, 2], True)
        )
